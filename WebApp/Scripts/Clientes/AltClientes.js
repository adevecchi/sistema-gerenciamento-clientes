...
$(document).ready(function () {
    ...
    if (beneficiarios.length) {
        for (var i = 0, size = beneficiarios.length; i < size; i++) {
            IncluirBeneficiarios(beneficiarios[i].CPF, beneficiarios[i].Nome);
        }
    }
        ...
        $.ajax({
            ...
            success:
                function (r) {
                    ...
                    $(".general-message").one("hide.bs.modal", function () {
...
function ModalDialog(titulo, texto) {
    ...
    var texto = '<div id="' + random + '" class="modal fade general-message">                                                               ' +
...
function verificarCPF(cpf) {
    ...
    // Remove os pontos/traço, caso exista
    cpf = cpf.replace(/[^\d]+/g, '');
    if (cpf == '') return false;
